...
consumerElectronicsDataForAnalysisDayAggregation$cameraAccesorryPropertionate <- consumerElectronicsDataForAnalysisDayAggregation$totalCameraAccessoryOrders / consumerElectronicsDataForAnalysisDayAggregation$totalOrders
...
computeInvestment <- function(record) {
  ...
  proportionate <- as.double(record['cameraAccesorryPropertionate'])
...
weekAggregationSplit2 <- consumerElectronicsDataForAnalysisDayAggregation %>% dplyr::select(-c(Month, deliverycdays_mean, deliverybdays_mean, product_procurement_sla_mean, cameraAccesorryPropertionate, gmv, offer_price, day, investment, investmentTV, investmentDigital, investmentSponsorship, investmentContentMarketing, investmentOnlinemarketing, investmentAffiliates, investmentSEM, investmentRadio, investmentOther)) %>% group_by(Year, week) %>% summarise_all(funs(sum = sum), na.rm = TRUE)
weekAggregationSplit3 <- consumerElectronicsDataForAnalysisDayAggregation %>% dplyr::select(Year, week, deliverycdays_mean, deliverybdays_mean, product_procurement_sla_mean, cameraAccesorryPropertionate) %>% group_by(Year, week) %>% summarise_all(funs(mean), na.rm = TRUE)
